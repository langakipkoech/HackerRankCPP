    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            ...
            for (int d = 0; d < DIRECTIONS; d++) {
                if (grid[i][j].state[d] == 1) {
                    density += 1;
                    velocity_x += direction_vectors[d][0];
                    velocity_y += direction_vectors[d][1];
                }
            }

            macro_grid[i][j].density = density;
            if (density > 0) {
                macro_grid[i][j].velocity_x = velocity_x / density;
                macro_grid[i][j].velocity_y = velocity_y / density;
            } else {
                macro_grid[i][j].velocity_x = 0;
                macro_grid[i][j].velocity_y = 0;
            }
        }
    }
}
...
void save_macroscopic_to_file(int timestep) {
    char filename[50];
    sprintf(filename, "macroscopic_t%d.txt", timestep);
    FILE *file = fopen(filename, "w");
    if (!file) {
...
int main() {
    ...
    //apply the boundary conditions
        ...
        //comopute macroscopic quantities
        compute_macroscopic();

        save_macroscopic_to_file(timestep);
    }
