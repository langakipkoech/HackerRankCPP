...
void save_grid_to_file(const char* base_filename, int rank);
void save_macroscopic_to_file(const char* base_filename, int rank);
 

void allocate_grids() {
    grid = malloc(M * sizeof(Node*));
    temp_grid = malloc(M * sizeof(Node*));
    macro_grid = malloc(M * sizeof(Macroscopic*));

    if (!grid || !temp_grid || !macro_grid) {
    ...
    for (int i = 0; i < M; i++) {
        ...
        macro_grid[i] = calloc(N, sizeof(Macroscopic));

        if (!grid[i] || !temp_grid[i] || !macro_grid[i]) {
        ...
        for (int j = 0; j < N; j++) {
            ...
            for (int d = 0; d < DIRECTIONS; d++) {
                if (grid[i][j].state[d] == 1) {
                    density += 1;
                    velocity_x += direction_vectors[d][1];
                    velocity_y += direction_vectors[d][0];
                }
            }

            macro_grid[i][j].density = density;
            macro_grid[i][j].velocity_x = (density > 0) ? velocity_x / density : 0;
            macro_grid[i][j].velocity_y = (density > 0) ? velocity_y / density : 0;
        }
    }
}
...
void save_grid_to_file(const char* base_filename, int rank) {
    char filename[100];
    sprintf(filename, "%s_rank_%d.txt", base_filename, rank); // Append rank to filename
...
void save_macroscopic_to_file(const char* base_filename, int rank) {
    ...
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            fprintf(file, "(%d,%d): Density=%.2f, Velocity=(%.2f, %.2f)\n", 
                    i, j, 
                    macro_grid[i][j].density, 
                    macro_grid[i][j].velocity_x, 
                    macro_grid[i][j].velocity_y);
        }
    }

    fclose(file);
    printf("Macroscopic quantities saved to %s\n", filename);
}
...
int main(int argc, char* argv[]) {
    ...
    for (int timestep = 0; timestep < TIMESTEPS; timestep++) {
        ...
        sprintf(config_filename, "config_timestep_%d.txt", timestep);
        save_grid_to_file(config_filename, rank);

        // Compute and save macroscopic quantities
        compute_macroscopic_quantities();
        char macro_filename[50];
        sprintf(macro_filename, "macroscopic_timestep_%d.txt", timestep);
        save_macroscopic_to_file(macro_filename, rank);
    }
